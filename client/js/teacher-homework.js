...
emptyHomeworkRow = emptyHomeworkRow + "        <input type='text' class='form-control hwName' style='height:40px; width:140px; font-family: monsterrat;' placeholder='Homework Name'/>";
emptyHomeworkRow = emptyHomeworkRow + "    </td>";
emptyHomeworkRow = emptyHomeworkRow + "    <td class='tdHWDueDate'>";
emptyHomeworkRow = emptyHomeworkRow + "        <input type='datetime-local' class='form-control hwDueDate' style='height:40px; width:140px; font-family: monsterrat;'/>";
emptyHomeworkRow = emptyHomeworkRow + "    </td>";
emptyHomeworkRow = emptyHomeworkRow + "    <td class='tdHWDescription'>";
emptyHomeworkRow = emptyHomeworkRow + "        <input type='text' class='form-control hwDescription' style='height:40px; width:140px; font-family: monsterrat;' placeholder='Homework Short Description'/>";
emptyHomeworkRow = emptyHomeworkRow + "    </td>";
emptyHomeworkRow = emptyHomeworkRow + "    <td class='tdHWDocuments'>";
emptyHomeworkRow = emptyHomeworkRow + "        <input type='file' multiple class='form-control hwDocuments' style='padding-top:10px; height:40px; width:140px; font-family: monsterrat;'/>";
emptyHomeworkRow = emptyHomeworkRow + "    </td>";
emptyHomeworkRow = emptyHomeworkRow + "    <td class='tdHwAction'>";
emptyHomeworkRow = emptyHomeworkRow + "        <a title='Save' class='btn border-shadow save'><span class='text-gradient'><i class='fas fa fa-floppy-o'></i></span></a>";
...
$(document).ready(function () {
    ...
    $("#btnAddHW").click(function () { 
        $("#tblData tbody").append(emptyHomeworkRow); // appending dynamic string to table tbody
    });
    ...
    $('#tblData').on('click', '.save', async function () {
        ...
        const hwDocuments =  $(this).parent().parent().find(".hwDocuments").val();
        ...
        const result = fetch('/api/addhomework', {
            ...
            method: "POST",
            body: JSON.stringify({hwName, hwDueDate, hwDescription, invitationCode, hwDocuments})
        })
        .then(res => res.json());
    ...
    $('#tblData').on('click', '.edit', async function () { 
        ...
        .html("<input type='text' value='"+hwName+"' class='form-control hwName' style='height:40px; width:120px; font-family: monsterrat;' placeholder='Homework Name'/>"); 

        const hwDueDate =$(this).parent().parent().find(".tdHWDueDate").html();
        console.log(hwDueDate)
        $(this).parent().parent().find(".tdHWDueDate")
        .html("<input type='datetime-local' value='"+hwDueDate+"' class='form-control hwDueDate' style='height:40px; width:120px; font-family: monsterrat;' placeholder='Homework Name'/>"); 

        const hwDescription =$(this).parent().parent().find(".tdHWDescription").html();
        $(this).parent().parent().find(".tdHWDescription")
        .html("<input type='text'  value='"+hwDescription+"'  class='form-control hwDescription' style='height:40px; width:120px; font-family: monsterrat;' placeholder='Homework Short Description'/>"); 
    ...
    $('#tblData').on('click', '.update', async function () { 
        ...
        const result = fetch('/api/updatehomework', {
            ...
            method: "POST",
            body: JSON.stringify({newHwName, newHwDueDate, newHwDescription, hwID, invitationCode})
        })
        .then(res => res.json());
